...
from .models import Info,Record,Post
...
def upload(request):
    if request.method == 'POST':
        ...
        global img_path
        img_path = Record.objects.create(SubCode=scode,Img=imgFile)
...
def generate_xls(request):
    ...
    # response['Content-Disposition'] = 'attachment; filename=%s' % smart_str('Result')
    # response['X-Sendfile'] = smart_str(xls_path)
    # return response
